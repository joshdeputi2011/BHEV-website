...
export default function MapplsStationMap({
  ...
  // Initialize Leaflet map with tiles and cluster group
  useEffect(() => {
    ...
    const tileLayer = L.tileLayer(tileUrl, {
      maxZoom: 19,
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a> • Powered by MapmyIndia / Mappls'
    }).addTo(map);
  ...
  // Update tile layer when theme toggles
  useEffect(() => {
  ...
  // Sync user location marker & auto-center on user location
  useEffect(() => {
    const map = mapRef.current;
    if (!map) return;
    ...
    if (userLocation && userLocation.lat && userLocation.lng) {
      ...
      // Smoothly pan/zoom to user location on initial locate
  ...
  // Fit bounds when requested or fitTrigger changes
  useEffect(() => {
    const map = mapRef.current;
    if (!map || !stations.length || fitTrigger === 0) return;

    let minLat = 90, maxLat = -90, minLng = 180, maxLng = -180;
    let count = 0;

    for (let i = 0; i < Math.min(stations.length, 5000); i++) {
      const s = stations[i];
      const lat = Number(s.latitude);
      const lng = Number(s.longitude);
    ...
    <div className="bhev-map-container">
      ...
      <div className="bhev-map-badge">
        <div className="bhev-map-badge__header">
          ...
          <strong>Official BEE & UEI Discovery Grid</strong>
        </div>
        <p>
          29,000+ national EV charging points across India. Powered by MapmyIndia / Mappls.
        </p>
      </div>
    </div>
  );
